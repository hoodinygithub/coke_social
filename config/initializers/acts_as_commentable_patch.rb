  ...
  def self.valid(options = {})
    find_options = { 
      :joins => 'INNER JOIN playlists ON comments.commentable_id = playlists.id 
                 ...
                 LEFT JOIN blocks ON blocks.blocker_id = playlists.owner_id', 
      :conditions => 'playlists.deleted_at IS NULL AND a1.deleted_at IS NULL AND (blocks.blockee_id IS NULL OR blocks.blockee_id <> comments.user_id)' }
