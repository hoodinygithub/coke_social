...
#      :subscriptions => my_subscriptions_path,
  ...
  def user_navigation
    ...
    # HACK for Registration and forgot  page
    return if params[:controller] == 'users' && ['new','create','forgot'].include?(params[:action])
    ...
    links.each do |link|
      css_class = ""
      actual_url = request.request_uri.split("?")[0]
      css_class << " active" if (link[:url].match(Regexp.new(actual_url + "$")) or params[:controller] == link[:menu].to_s)
