  ...
  if @station.is_a? EditorialStation    
    @songs.sort_by { rand }.each do |song|
      if song && song.artist
        xml.song do
          xml.idsong song.id
          xml.album_id song.album_id
          xml.idpl @station.id
          xml.idband song.artist.id
          xml.station_id song.artist.station.nil? ? 0 : song.artist.station.id
    ...
    @songs.each do |song|
      station = 0
      unless artists_checked.include?(song.artist_id)
        station = AbstractStation.find_by_artist_id(song.artist_id) 
        artists_checked << song.artist_id
      end
      xml.song do
        xml.idsong song.id
        xml.title song.title
        ...
        xml.album_id song.album_id
        xml.station_id station.nil? ? 0 : station.id
        xml.duration song.duration
